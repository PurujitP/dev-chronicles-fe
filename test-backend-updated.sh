#!/bin/bash
echo "=== DevChronicles Refresh Token Test (Updated) ==="
echo
...
REFRESH_STATUS=$(curl -s -o /dev/null -w "%{http_code}" -X POST http://127.0.0.1:8000/backend-api/auth/refresh -H "Content-Type: application/json" -d '{"refresh_token":"test"}')
...
if [ "$REFRESH_STATUS" = "404" ]; then
    ...
    echo "   Please add POST /backend-api/auth/refresh to your backend"
...
echo
echo "=== Testing the correct endpoint now: /refresh ==="
REFRESH_CORRECT=$(curl -s -o /dev/null -w "%{http_code}" -X POST http://127.0.0.1:8000/backend-api/auth/refresh -H "Content-Type: application/json" -d '{"refresh_token":"test"}')
echo "Correct refresh endpoint (/refresh): $REFRESH_CORRECT"
